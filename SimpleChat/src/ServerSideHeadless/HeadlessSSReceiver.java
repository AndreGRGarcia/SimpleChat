...
public class HeadlessSSReceiver extends Thread {
	
	private ServerHeadless server;
	private HeadlessSSSender sender;
	private Socket socket;
	private ObjectInputStream in;
	private int id;
	private String filesLocation;
	
	public HeadlessSSReceiver(ServerHeadless server, Socket socket, int id, String filesLocation) {
		this.server = server;
		this.socket = socket;
		this.id = id;
		...
		try{	
			for(;;) {
				...
				if(msg.getFunction() == Message.SEND_SOUND_FILES) {
					...
					for(HeadlessSSSender s: server.getList()) {
						s.send(msg);
					}
				}
			}
	...
	private void sendSoundFiles() {
		try {
			...
			System.out.println("Error with connection to client");
			System.exit(1);
		} catch (ClassNotFoundException e) {
			System.out.println("Error on receiving message from client");
			System.exit(1);
		}
	...
	private void closeConnections() {
		try {
			...
			socket.close();
			System.out.println("Client " + getIdd() + " disconnected with success");
		} catch(IOException e) {
			System.out.println("Something went wrong with closing connection with client " + getIdd());
		}
	}
	...
	public void setSender(HeadlessSSSender sender) {
		this.sender = sender;
	}
